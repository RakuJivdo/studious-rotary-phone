...
MAX_OPTION_LEN = 100
# Explanation ki limit ab unlimited hai kyunki ye poll ke andar nahi ja rhi!
FIXED_OPTIONS = 4
...
def parse_block(block: str) -> dict:
    ...
    Line 6 onward -> Explanation (Compulsory, no upper limit, sent as spoiler)
    """
    ...
    # ── Total lines validate karna (Min 6 lines: 1 Q + 4 Opt + Min 1 Exp) ──
    ...
    # ── Lines 6+: Explanation (Saari bachi hui lines jab tak gap na aaye) ───
    explanation_lines = non_empty_lines[5:]
    # Sabhi lines ko join karke original line breaks ko preserve rakhenge
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for block in blocks:
        ...
        if result["ok"]:
            try:
                # 1. Pehle pure 5 lines ka Poll bhejenge (explanation parameter ke bina)
                ...
                # 2. Poll ke turant baad explanation ko Spoiler format mein bhejenge
                # MarkdownV2 me spoiler ke liye || use hota hai
            ...
            # Error hone par separate split messages (Pehle jaisa hi)
    ...
    # Agar koi error nahi hai toh original message delete ho jayega
    if error_count == 0:
        try:
